...
<div class="x_panel">
    ...
    <div class="x_content">
      ...
      <div class="form-group">
        ...
        <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Jurisdicción: <span class="required">*</span>
            </label>
            <div class="col-md-6 col-sm-6 col-xs-12">
            <select class="select2_single form-control" tabindex="-1" name="jurisdiccion">
            
                    <option value="0" selected>--SELECCIONAR--</option>
                    @foreach($jurisdictions as $items)
                
                        @if(old('jurisdiccion'))
                                @if(old('jurisdiccion') == $items->id)
                                        <option value="{{$items->id}}" selected>{{$items->nombre}}</option>
                                @else
                                        <option value="{{$items->id}}" >{{$items->nombre}}</option>
                                @endif
